...
use crate::mcp::config::{McpTransport, McpConfigCollection, ScopedMcpServerConfig};
use crate::mcp::mcp::mcp_tool_name;
...
impl McpServerManager {
    #[must_use]
    pub fn from_runtime_config(config: &McpConfigCollection) -> Self {
        Self::from_servers(&config.servers)
    }
    ...
    pub fn from_servers(servers: &BTreeMap<String, ScopedMcpServerConfig>) -> Self {
        ...
        for (server_name, server_config) in servers {
            if server_config.config.transport() == McpTransport::Stdio {
                ...
                unsupported_servers.push(UnsupportedMcpServer {
                    ...
                    reason: format!(
                        ...
                        server_config.config.transport()
                    ),
                });
            }
        }
